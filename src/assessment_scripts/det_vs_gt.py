...
if __name__ == "__main__":
    ...
    tagged_gt_gdf, tagged_dets_gdf = tag(gt=gt_trees_gdf, preds=dets_gdf, tol=tolerance_m)
    logger.info("<- ...done.")
    ...
    for sector in sorted(gt_sectors_gdf.sector.unique()):
        metrics = assess(
            ...
            tagged_preds = tagged_dets_gdf[tagged_dets_gdf.sector == sector],
        )
        print(",".join([f"{k}={v:.3f}" for k, v in metrics.items()] + [f"sector={sector}"]))
    logger.info("<-- ...done.")
    logger.info("<- ...done.")
    logger.info("< ...done.")
